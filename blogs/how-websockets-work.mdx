...
canonicalUrl: "https://www.deepintodev.com/blog/how-websockets-work"
...
When **HTTP 1.0** was invented, it was designed as a simple request–response system: the client makes a request, and the server sends back a response. In this setup, the client is always the one that has to initiate the _request–response cycle._
...
**_Upgrade: websocket_** -> this means, “let’s upgrade the HTTP protocol to WebSocket.”

**_Connection: Upgrade_** -> this means, “the connection type will change for this communication.”

**_Sec-WebSocket-Key_** -> this is a unique key sent by the client during the handshake, so the server can verify and confirm that it’s a real WebSocket connection.
...
**_101 Switching Protocols_** -> 101 is the HTTP status code indicating that the server agrees to switch to a different protocol requested by the client (e.g., WebSocket), just like other HTTP status codes such as 200 or 404.

**_Sec-WebSocket-Accept_** -> with this key, the server tells the client, “I am a legitimate WebSocket server.”
