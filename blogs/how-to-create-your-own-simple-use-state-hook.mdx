...
description: "Learn how React hooks work under the hood by building a simplified useState. This guide explains closures, state persistence, and exactly why the 'Rules of Hooks' are strictly enforced."
date: "2025-06-10"
dateModified: "2025-12-20"
author: "Kaan Peksen"
category: "React"
tags: ["react", "how-react-works", "use-state", "hooks", "closure"]
...
Not only will you learn how to create a simplified version of the `useState` function, but you will also learn why those hook rules that you've always followed exist. (_Rules like they can only be defined at the top level, or they can only be defined inside of the react components_.) So, after this short blog, you will actually understand the foundation of `useState`, and why you had to follow all those rules that you've always been told.

Now, let’s create two buttons (_Button A and Button B_) and implement the increment logic using our own custom useState hook instead of React’s built-in one. Basically, clicking these buttons should increase the value inside them.
...
At this point, I assume you already know what the `useState` hook does. It's actually **just a function** that takes an initial value and returns a state and a function to change that state. So, we’ll start by creating a function that behaves the same way, since our goal is to create our own `useState` hook.
...
Right now, our `useState` just takes an initial value and returns that value along with a function. This function will be used to update the state, exactly like you know from React's built-in `useState`.
...
But of course, we don't want to return the _**initialValue**_ from our `useState` function, because later on we’ll be changing the value of A. We always need to return the latest value, not the initial value.

So, let's create a different variable called _**stateValue**_, and return this value instead of _initialValue_. But where should we create this variable? As you can guess, if we create it inside of the `useState` function, we’ll be resetting _stateValue_ every time the function is called. So, we need some kind of persistence across renders. In other words, we don't want it to be re-initialized each time `useState` is called. That’s why we’re going to create that variable outside of the scope of the `useState` function.
...
We basically say: If _**stateValue**_ is undefined, use the _**initialValue**_; if not, use the most up-to-date state value and return it from the `useState` function. It's quite simple, right?
...
When we call the first `useState`, **callIndex** will be 0, and **stateValues[0]** will be equal to the initial value of the first `useState`. When we call it a second time, callIndex will be 1, and the same thing will happen.
...
As you can see, we're directly accessing callIndex to change the value of our state. Think about it, when we call the useState function for the second time, callIndex will be equal to 1. **So, every time we access callIndex, we're always accessing index 1**. We can never reach the 0th index of the array, which means we're not able to change the value of Button A.
